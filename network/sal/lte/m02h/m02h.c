...
static int m02h_lte_get_apn(char *apn, int apn_len)
{
...
static int m02h_lte_got_ip(const char *apn)
{
    char cmd[M02H_DEFAULT_CMD_LEN] = {0};
    char rsp[M02H_LARGE_RSP_LEN] = {0};
    char ipaddr[16] = {0};
...
static int m02h_lte_module_init(void)
{
    ...
    while (1) {
        ...
        if (papn == NULL) {
            ret = m02h_lte_get_apn(apn, M02H_APN_LEN);
            if (ret) {
                LOGE(TAG, "%s %d failed \r\n", __func__, __LINE__);
                goto err;
            }
