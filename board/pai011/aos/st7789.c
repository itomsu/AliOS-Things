...
void ST7789H2_SetCursor(uint16_t Xpos, uint16_t Ypos)
{
  ...
  parameter[0] = 0x00 + Xpos >> 8;     
  parameter[1] = 0x00 + Xpos & 0xFF;
  parameter[2] = 0x00 + Xpos >> 8;
  parameter[3] = 0xFF + Xpos & 0xFF;
  ST7789H2_WriteReg(0x2A, parameter, 4);
  /* RASET: Row Addrses Set */  
  parameter[0] = 0x00 + Ypos >> 8;
  parameter[1] = 0x00 + Ypos & 0xFF;
  parameter[2] = 0x00 + Ypos >> 8;
  parameter[3] = 0xFF + Ypos & 0xFF;
  ST7789H2_WriteReg(0x2B, parameter, 4);
}

uint8_t black_gui[WIDTH<<1] = {0};
...
void BSP_LCD_Clear(uint16_t Color)
{
  ...
  for (counter = 0; counter < HEIGHT; counter++)
  {
		...
		LcdWriteDataMultiple(black_gui, sizeof black_gui);	 
  }
}
